...
class QuestionSeedDatabaseWorker(
        context: Context,
        workerParams: WorkerParameters
) : CoroutineWorker(context, workerParams) {
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        try {
            val filename = inputData.getString(QUESTION_KEY_FILENAME)
            if (filename != null) {
                applicationContext.assets.open(filename).use { inputStream ->
                    JsonReader(inputStream.reader()).use { jsonReader ->
                        val questionType = object : TypeToken<List<QuestionModel>>() {}.type
                        val questionList: List<QuestionModel> = Gson().fromJson(jsonReader, questionType)
    ...
    companion object {
        private const val TAG = "QuestionSeedDatabaseWorker"
        const val QUESTION_KEY_FILENAME = "QUESTION_DATA_FILENAME"
    }
}
